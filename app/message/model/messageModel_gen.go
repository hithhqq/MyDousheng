...
var (
	...
	cacheDoushengMessageIdPrefix = "cache:dousheng:message:id:"
)
...
func (m *defaultMessageModel) Delete(ctx context.Context, id int64) error {
	doushengMessageIdKey := fmt.Sprintf("%s%v", cacheDoushengMessageIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, doushengMessageIdKey)
	return err
}
...
func (m *defaultMessageModel) FindOne(ctx context.Context, id int64) (*Message, error) {
	doushengMessageIdKey := fmt.Sprintf("%s%v", cacheDoushengMessageIdPrefix, id)
	var resp Message
	err := m.QueryRowCtx(ctx, &resp, doushengMessageIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultMessageModel) Insert(ctx context.Context, data *Message) (sql.Result, error) {
	doushengMessageIdKey := fmt.Sprintf("%s%v", cacheDoushengMessageIdPrefix, data.Id)
...
func (m *defaultMessageModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheDoushengMessageIdPrefix, primary)
}
